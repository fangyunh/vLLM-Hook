#!/bin/bash
# run_hs_parity_full.sh — graph-vs-eager HIDDEN-STATE parity under FULL
...
echo "[run_hs_parity_full] cudagraph_mode=FULL capture=buffer"
...
run_leg () {
  ...
  echo "============================================================"
  echo "[run_hs_parity_full] === ${name} 1/3 graph capture (FULL, buffer, batched=$batched) ==="
