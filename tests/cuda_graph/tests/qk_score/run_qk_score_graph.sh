...
#
# Captures the score under op-mode (PIECEWISE) and buffer-mode (FULL),
...
# >1 so the buffer (FULL) arm actually REPLAYS decode cudagraphs (not just eager
...
echo "[run_qk_score_graph] === 2/5 buffer-mode (FULL) score ==="
VLLM_HOOK_SCORE_GRAPH=1 VLLM_HOOK_QK_CAPTURE=buffer VLLM_HOOK_CUDAGRAPH_MODE=FULL \
...
echo "[run_qk_score_graph] === 6/7 buffer-mode (FULL) last_token score (D1 on-GPU egress) ==="
VLLM_HOOK_SCORE_GRAPH=1 VLLM_HOOK_QK_CAPTURE=buffer VLLM_HOOK_CUDAGRAPH_MODE=FULL \
...
VLLM_HOOK_SCORE_MH=1 VLLM_HOOK_SCORE_GRAPH=1 VLLM_HOOK_QK_CAPTURE=buffer VLLM_HOOK_CUDAGRAPH_MODE=FULL \
