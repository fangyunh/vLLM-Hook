...
WHAT IT PROVES:
  ...
  The SCORE arm is path-selectable via env (so one harness covers all three paths):
    ...
    buffer : VLLM_HOOK_QK_SCORE=1, ALLOW_CUDAGRAPH=1, VLLM_HOOK_QK_CAPTURE=buffer (FULL)

USAGE (three sequential processes in one LSF job; see run_qk_score_graph.sh):
