...
STALE — NEEDS REVIEW BEFORE TRUSTING RESULTS: this oracle configures its probes via the
`VLLM_HOOK_CAPTURE_*` memory-manager knobs (`CaptureDrainManager` and friends), which have
since been removed from the package. Any probe here that depends on those knobs no longer
exercises anything real; it needs a rewrite by someone running the LSF campaign before its
