#!/bin/bash
# run_qk_parity_full.sh — graph-vs-eager QK parity under FULL (v0.4.0 M2):
...
echo "[run_qk_parity_full] cudagraph_mode=FULL capture=buffer config=$CFG max_tokens=$VLLM_HOOK_PARITY_MAX_TOKENS"
...
echo "============================================================"
echo "[run_qk_parity_full] === A 1/3 graph capture (FULL, buffer) ==="
...
echo "============================================================"
echo "[run_qk_parity_full] === B 1/3 graph capture (FULL, buffer, decode) ==="
...
# ---- leg C: the PER-REQUEST fallback — same as leg B but VLLM_HOOK_BATCHED_EGRESS=0.
# Lever A (batched) became the DEFAULT in 201544, so legs A/B above now exercise it and this
# leg pins the flag OFF to keep the per-request clone path covered. (Before the default flip
# this leg was the =1 case and A/B were the fallback — the roles are simply swapped; both
# paths stay proven byte-identical against the SAME eager ground truth EB, which uses op mode
# and ignores _BATCHED_EGRESS either way.) Covers q (incl. last-token sparsity) + k_all.
GC="$WORK/both_graph_perreq.pkl"
echo "============================================================"
echo "[run_qk_parity_full] LEG C = per-request fallback (hooks_on=both, VLLM_HOOK_BATCHED_EGRESS=0)"
echo "============================================================"
echo "[run_qk_parity_full] === C 1/2 graph capture (FULL, buffer, per-request egress) ==="
VLLM_HOOK_PARITY_HOOKS_ON=both VLLM_HOOK_QK_CAPTURE=buffer VLLM_HOOK_BATCHED_EGRESS=0 \
  python -u "$PY" capture --mode graph --out "$GC"
echo "[run_qk_parity_full] === C 2/2 compare per-request-graph vs eager ==="
