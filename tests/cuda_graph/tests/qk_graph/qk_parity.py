...
WHY A SEPARATE HARNESS (not the in-process VLLM_HOOK_PARITY arm):
  ...
  process (the in-process `--parity` arm) fails with
...
WHAT IT PROVES (the plan's Step 1 pass criterion):
  The post-RoPE Q/K captured under CUDA graphs (the buffer path: the static-buffer
  capture_qk scatter absorbed into the graph) is numerically equal — within
  tolerance — to the Q/K captured by the LEGACY v0.2.0 eager `register_forward_hook`
  path, per layer, per request. That is the difference between "the op fires and
  scores look plausible" and "the captured tensors are correct."
...
USAGE (driven by run_qk_parity_full.sh as three sequential processes in one job):
...
def capture(mode, out_path, scenario="fresh"):
    ...
    if mode == "graph" and os.environ.get("VLLM_HOOK_AOT_PROBE") == "1":
        ...
        report_aot_counters(llm, tag=f"parity:{mode}")
