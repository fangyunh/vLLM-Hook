"""Integration core for CUDA-graph HIDDEN-STATE capture (op mode).

This mirrors the QK op-mode path (graph/install.py) for the hidden-states worker.
The only structural differences from QK:

  * Wrap point: the DECODER LAYER class (`model.layers.<i>`), not the attention
    module — and capture its OUTPUT (after the original forward), not the inputs.
    This is the exact point step0_8 proved lands in the captured graph.
  * The residual stream: vLLM decoder layers use a fused-residual pattern and
    return ``(hidden_states, residual)``; the captured hidden state is their sum
    (matching the eager ``hs_hook``: ``output[0] + output[1]``). We pass both
    tensors plus a ``has_residual`` flag to the op and form the sum in the
    capture body (eager seam), so no extra tensor is allocated inside the graph.
  * No prefix-K: hidden states only cover the tokens actually computed this step.

Everything else (custom op as a splitting-op eager seam, install at load_model
before compile, REUSE CONTRACT into the same worker buckets) is identical to QK,
so get_captured_states / flush_disk / _save_safetensors / the analyzer are
unchanged.
"""
...
# Class-wrap bookkeeping (idempotent + reversible), mirroring the QK path.
_WRAPPED_LAYER_CLASSES: Dict[type, Any] = {}
_HS_LAYER_ATTR = "_vllm_hook_hs_layer"   # per decoder-layer instance: 1-based layer num (-1 => skip)
_GLOBAL_SINK_HS = None                    # int64 (1,) keep-alive for hs_probe

# Set at install for the impl-runs capture body.
_ACTIVE_WORKER_HS = None
_LAYER_TO_NAME_HS: Dict[int, str] = {}    # 1-based layer num -> module name (egress key)
...
def _capture_body_hs(hidden: torch.Tensor, residual: torch.Tensor,
    ...
    """The eager hs_hook body, run INSIDE the hs_probe op (op mode).

    Runs mid-forward (live forward context + current input_batch), forms the
    residual stream, and writes the SAME worker buckets the eager path uses.
    ...
    # query_start_loc: prefer attn metadata; fall back to the model_runner's
    # persistent CPU buffers under torch.compile (where attn_metadata is None).
    ...
    # Residual stream: hidden + residual (fused-residual layers), or just hidden.
    ...
    for i in range(bs):
        ...
        # Layer filter: output_hidden_states is True (all) or a 1-based list.
        output_layers = extra.get("output_hidden_states")
        if isinstance(output_layers, list) and layer_idx not in output_layers:
            continue

        # hooks_on prefill/decode gating (current step state — correct mid-forward).
...
def _wrap_layer_class(cls: type) -> None:
    ...
    Idempotent per class. Runs AFTER the original forward (we need its output);
    the op is recorded as a splitting-op seam right after the layer. Reads a plain
    int layer attribute off the instance (install-time constant; -1 => skip), so
    no per-instance object is touched inside the traced forward (mirrors QK).
    """
...
def install_hs_hosts(worker) -> None:
    """Install the op-mode hidden-state capture path (decoder-layer wrap).

    Runs from the load_model patch (model built, before compile/capture). On the
    capture rank it tags each decoder layer with its 1-based number, maps
    layer->module name, creates the keep-alive sink, class-wraps the layer, and
    registers hs_probe as a splitting op. Capture happens inside the op impl.
    """
    ...
    # _conf is consumed by get_captured_states / flush_disk — match the eager
    # worker EXACTLY.
    ...
    if should_capture:
        ...
        for name, module, layer_num0 in matched:
            ln = layer_num0 + 1  # expose 1-based (HuggingFace/Eagle convention)
    ...
    # Ensure hs_probe is a splitting op on the WORKER's resolved config (runs at
    # load_model, before compile) — otherwise the op is absorbed into a replayed
    # cudagraph segment and its impl is skipped (the load-bearing fix, same as QK).
    try:
