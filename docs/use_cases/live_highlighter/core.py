...
class TokenHighlighterVisualizer:
    def __init__(self, model: HookLLM):
        self.model = model
    ...
    def _target_ids(self, phrase: str) -> None:
        cfg = self.model._highlighter_config
        if not cfg:
            raise RuntimeError("HookLLM needs token_highlighter config_file.")
        cfg["target_token_ids"] = self.model.tokenizer.encode(phrase, add_special_tokens=False)

    def mitigate(self, prompt: str, phrase: str, beta: float, *, max_tokens: int = 32) -> str:
        self.model._highlighter_config["beta"] = float(beta)
    ...
    def _highlight(
        ...
        *,
        ...
        cap = self.model.generate(
            prompt, highlighter_mode="capture", temperature=0.0, max_tokens=max_tokens
        )
        analysis = self.model.analyze(analyzer_spec={**_ANALYZER, **(analyzer_spec or {})})
    ...
    def _display_heatmap(self, payload: dict[str, Any]) -> None:
        uid = f"viz-{uuid.uuid4().hex[:8]}"
        body = (
            self._html.replace("viz-root-uuid", uid)
            .replace("DATA_PLACEHOLDER", json.dumps(payload))
            .replace("/*SCRIPT_PLACEHOLDER*/", self._js.replace("viz-root-uuid", uid))
        )
        doc = (
            f"<!DOCTYPE html><html><head><meta charset='utf-8'><style>{self._css}</style></head>"
    ...
    def _beta_slider(
        ...
        *,
        ...
        slider = w.FloatSlider(
            value=float(self.model._highlighter_config.get("beta", 0.1)),
        ...
        def _on(change: dict) -> None:
            ...
            with out:
                ...
                if not p or not ph:
                    print("Enter prompt and target phrase first.")
                    return
                try:
                    text = self.mitigate(p, ph, change["new"], max_tokens=max_tokens)
                    print(f"Mitigated (β={change['new']:.2f}):\n{text}")
                except Exception as e:
                    print(f"Mitigate error: {e}")

        slider.observe(_on, names="value")
        box = w.VBox([
    ...
    def visualize(
        ...
        *,
        ...
        if not interactive:
            ...
            if mitigate_slider:
                self._beta_slider(static_read, max_tokens=max_tokens)
            return
        ...
        heat_out = w.Output(layout=w.Layout(width="98%", overflow="visible"))
        extra_out = w.Output()
        ...
        def _run(_: Any = None) -> None:
            ...
            with heat_out:
                ...
                if not p or not t:
                    ...
                    return
                try:
                    self._display_heatmap(self._highlight(p, t, **kw))
                except Exception as e:
            ...
            if mitigate_slider:
                with extra_out:
                    clear_output(wait=True)
                    self._beta_slider(read, max_tokens=max_tokens, parent=extra_out)

        btn.on_click(_run)
        display(w.VBox([
            w.HTML("<b>Prompt</b>"), prompt_w,
            w.HTML("<b>Target phrase</b>"), target_w,
            btn, heat_out, extra_out,
        ]))
